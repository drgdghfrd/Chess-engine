...
#endif
// v1.0.9: x86 vectorised accumulator update (sign-extend int8 weights -> int16 add/sub).
// AVX2 processes 16 int16 per iteration; SSE4.1 processes 8. Both beat the scalar
// 8-at-a-time loop on every modern desktop x86_64 build.
...
[[maybe_unused]] static bool cpuHasDotProd() {
...
static inline void applyColumn(int16_t* __restrict acc,
    ...
    // 256-bit: 16 int16 per iteration. pmovsxbw sign-extends 16 int8 -> 16 int16.
    if (sign > 0) {
        for (int h = 0; h < NNUE::HIDDEN_SIZE; h += 16) {
            __m256i a = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(acc + h));
            __m128i c8 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(col + h));
            __m256i c16 = _mm256_cvtepi8_epi16(c8);
            _mm256_storeu_si256(reinterpret_cast<__m256i*>(acc + h), _mm256_add_epi16(a, c16));
        }
    } else {
        for (int h = 0; h < NNUE::HIDDEN_SIZE; h += 16) {
            __m256i a = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(acc + h));
            __m128i c8 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(col + h));
            __m256i c16 = _mm256_cvtepi8_epi16(c8);
            _mm256_storeu_si256(reinterpret_cast<__m256i*>(acc + h), _mm256_sub_epi16(a, c16));
        }
    }
#elif defined(__SSE4_1__)
    // 128-bit: 8 int16 per iteration, vectorised (vs scalar unrolled below).
    if (sign > 0) {
        for (int h = 0; h < NNUE::HIDDEN_SIZE; h += 8) {
            __m128i a = _mm_loadu_si128(reinterpret_cast<const __m128i*>(acc + h));
            __m128i c8 = _mm_loadl_epi64(reinterpret_cast<const __m128i*>(col + h));
            __m128i c16 = _mm_cvtepi8_epi16(c8);
            _mm_storeu_si128(reinterpret_cast<__m128i*>(acc + h), _mm_add_epi16(a, c16));
        }
    } else {
        for (int h = 0; h < NNUE::HIDDEN_SIZE; h += 8) {
            __m128i a = _mm_loadu_si128(reinterpret_cast<const __m128i*>(acc + h));
            __m128i c8 = _mm_loadl_epi64(reinterpret_cast<const __m128i*>(col + h));
            __m128i c16 = _mm_cvtepi8_epi16(c8);
            _mm_storeu_si128(reinterpret_cast<__m128i*>(acc + h), _mm_sub_epi16(a, c16));
        }
    }
#else
    if (sign > 0) {
...
bool NNUE::loadMemory(const void* data, std::size_t size, const char* source) {
    ...
    const bool isMulti  = (std::strncmp(h.magic, "CZNNUE64", 8) == 0);
    const bool isSingle = (std::strncmp(h.magic, "CZNNUE32", 8) == 0);
    if (!isMulti && !isSingle) return false;
    if (h.inputs != INPUTS || h.hidden != HIDDEN_SIZE || h.output != 1) return false;
    if (isSingle && h.version != 32) return false;
    if (isMulti  && h.version != 64) return false;
    ...
    if (size < offset + expectBias + expectWeights) return false;
    ...
    arch_ = isMulti ? Arch::Multi : Arch::Single;

    if (isSingle) {
        const size_t expectOutput = static_cast<size_t>(HIDDEN_SIZE) * sizeof(int8_t);
        if (size < offset + expectOutput + sizeof(int32_t)) return false;
        std::memcpy(outputWeights_.data(), base + offset, expectOutput);
        offset += expectOutput;
        std::memcpy(&outputBias_, base + offset, sizeof(int32_t));
    } else {
        // Multi-layer (HalfKAv2-style): L1 concat (stm-first) -> L2 -> L3 -> scalar.
        const size_t expectL2w = static_cast<size_t>(2 * HIDDEN_SIZE) * L2_SIZE * sizeof(int8_t);
        const size_t expectL2b = static_cast<size_t>(L2_SIZE) * sizeof(int32_t);
        const size_t expectL3w = static_cast<size_t>(L2_SIZE) * L3_SIZE * sizeof(int8_t);
        const size_t expectL3b = static_cast<size_t>(L3_SIZE) * sizeof(int32_t);
        const size_t expectOut = static_cast<size_t>(L3_SIZE) * sizeof(int8_t);
        const size_t need = expectL2w + expectL2b + expectL3w + expectL3b + expectOut + sizeof(int32_t);
        if (size < offset + need) return false;
        std::memcpy(l2Weights_.data(), base + offset, expectL2w); offset += expectL2w;
        std::memcpy(l2Bias_.data(),    base + offset, expectL2b); offset += expectL2b;
        std::memcpy(l3Weights_.data(), base + offset, expectL3w); offset += expectL3w;
        std::memcpy(l3Bias_.data(),    base + offset, expectL3b); offset += expectL3b;
        std::memcpy(outputWeights_.data(), base + offset, expectOut); offset += expectOut;
        std::memcpy(&outputBias_, base + offset, sizeof(int32_t));
    }
...
const char* NNUE::architectureName() const noexcept {
    return arch_ == Arch::Multi ? "HalfKAv2-512x8x32" : "HalfKP-40960x256";
}
...
bool NNUE::loadEmbedded() {
    ...
    return loadMemory(begin, size, "embedded:ChessZero-v1.0.9-current.nnue");
#else
    return false;
#endif
}
...
// v1.0.9 multi-layer forward pass. L1 accumulators (int16) are clipped to int8
// [0,127], concatenated with side-to-move first, then run through L2 (8) -> L3 (32)
// -> scalar. All intermediate activations are clipped ReLU, quantized to int8.
int NNUE::forwardMulti(const int16_t* stm, const int16_t* other) const {
    if (!loaded_ || !stm || !other) return 0;
    alignas(64) int8_t l1[2 * HIDDEN_SIZE];
    for (int h = 0; h < HIDDEN_SIZE; ++h) {
        int x = stm[h];   if (x < 0) x = 0; else if (x > 127) x = 127;
        l1[h] = static_cast<int8_t>(x);
        int y = other[h]; if (y < 0) y = 0; else if (y > 127) y = 127;
        l1[HIDDEN_SIZE + h] = static_cast<int8_t>(y);
    }
    // L2: 8 neurons, input = 2*HIDDEN_SIZE int8
    int8_t l2[L2_SIZE];
    for (int j = 0; j < L2_SIZE; ++j) {
        int32_t sum = l2Bias_[j];
        const int8_t* w = l2Weights_.data() + static_cast<size_t>(j) * 2 * HIDDEN_SIZE;
        for (int i = 0; i < 2 * HIDDEN_SIZE; ++i) sum += l1[i] * w[i];
        if (sum < 0) sum = 0; else if (sum > 127) sum = 127;
        l2[j] = static_cast<int8_t>(sum);
    }
    // L3: 32 neurons, input = L2_SIZE int8
    int8_t l3[L3_SIZE];
    for (int j = 0; j < L3_SIZE; ++j) {
        int32_t sum = l3Bias_[j];
        const int8_t* w = l3Weights_.data() + static_cast<size_t>(j) * L2_SIZE;
        for (int i = 0; i < L2_SIZE; ++i) sum += l2[i] * w[i];
        if (sum < 0) sum = 0; else if (sum > 127) sum = 127;
        l3[j] = static_cast<int8_t>(sum);
    }
    // Output: scalar from L3
    int32_t sum = outputBias_;
    for (int i = 0; i < L3_SIZE; ++i) sum += l3[i] * outputWeights_[i];
    if (outputShift_) sum >>= outputShift_;
    if (sum > 30000) return 30000;
    if (sum < -30000) return -30000;
    return static_cast<int>(sum);
}
...
int NNUE::evaluate(const Position& p, const Accumulator& acc) const {
    ...
    if (arch_ == Arch::Multi) {
        if (p.side() == 1) return forwardMulti(acc.white(), acc.black());
        else               return forwardMulti(acc.black(), acc.white());
    }
