# ChessZero v1.0.8 — Engine + Android production release

## v1.0.8 — Self-contained Android NNUE/OEX runtime

ChessZero v1.0.8 embeds the HalfKP-40960x256 network into the arm64 Android OEX executable and JNI library. Chessis/OEX launches no longer depend on filesystem access to the APK's private NNUE asset path. `EvalFile` also accepts `embedded://ChessZero-v0.75-halfkp.nnue`.
...
`nets/ChessZero-v0.75-halfkp.nnue` is the bundled HalfKP bootstrap network used by the current release. It is retained as the reproducible evaluation asset; this release does not claim a new independently measured Elo result.
